  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen py-12 px-4 bg-gray-100 dark:bg-[#1e1e1e]">
      ...
      <form className="space-y-4 w-full max-w-lg">
        ...
        {/* GitHub Achievements Builder Section */}
        <div className="p-6 bg-white dark:bg-[#121212] border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-black dark:text-white flex items-center gap-2">
          ...
          {achievements.enabled && (
            <div className="space-y-6 border-t border-gray-100 dark:border-gray-800 pt-4 transition-all duration-300">
              {/* Username Input for Achievements */}
              <div className="w-full md:w-64">
                <label htmlFor="achievements-username-input" className="block text-xs font-semibold mb-1 text-gray-500 dark:text-gray-400">GitHub Username</label>
                <Input
                  id="achievements-username-input"
              ...
              <div className="space-y-4">
                {achievements.order.map((widgetId, index) => {
                  ...
                  const activeUser = achievements.username || username || '';

                  // Compute live preview image URL
                  let previewUrl = '';
                  if (activeUser && widgetConfig.enabled) {
                    if (widgetId === 'trophy') {
                      previewUrl = `https://github-profile-trophy.vercel.app/?username=${activeUser}&theme=${widgetConfig.theme || 'flat'}${widgetConfig.noFrame ? '&no-frame=true' : ''}${widgetConfig.noBg ? '&no-bg=true' : ''}${widgetConfig.rows ? `&row=${widgetConfig.rows}` : ''}${widgetConfig.columns ? `&column=${widgetConfig.columns}` : ''}`;
                    } else if (widgetId === 'visitor') {
                      previewUrl = `https://komarev.com/ghpvc/?username=${activeUser}&color=${widgetConfig.color || '0078d7'}&style=${widgetConfig.style || 'flat'}`;
                    } else if (widgetId === 'snake') {
                      previewUrl = `https://raw.githubusercontent.com/${activeUser}/${activeUser}/output/github-contribution-grid-snake.svg`;
                    } else if (widgetId === 'graph') {
                      previewUrl = `https://github-readme-activity-graph.vercel.app/graph?username=${activeUser}&theme=${widgetConfig.theme || 'github'}${widgetConfig.hideBorder ? '&hide_border=true' : ''}`;
                    }
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Customize Options (Only if widget is enabled) */}
                      {widgetConfig.enabled && (
                        <div className="space-y-3">
                          ...
                          {widgetId === 'trophy' && (
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                              ...
                              <div>
                                <label className="block text-2xs uppercase tracking-wider text-gray-400 font-bold mb-0.5">Rows</label>
                                <input
                                  type="number"
                                  min="1"
                                  max="10"
                                  value={widgetConfig.rows || 1}
                                  onChange={(e) => updateWidgetProperty('rows', parseInt(e.target.value) || 1)}
                                  className="w-full text-xs p-1.5 rounded border border-gray-300 dark:bg-gray-800 dark:text-white dark:border-gray-600"
                                />
                              </div>
                              <div>
                                <label className="block text-2xs uppercase tracking-wider text-gray-400 font-bold mb-0.5">Columns</label>
                                <input
                                  type="number"
                                  min="1"
                                  max="10"
                                  value={widgetConfig.columns || 6}
                                  onChange={(e) => updateWidgetProperty('columns', parseInt(e.target.value) || 6)}
                                  className="w-full text-xs p-1.5 rounded border border-gray-300 dark:bg-gray-800 dark:text-white dark:border-gray-600"
                                />
                              </div>
                              <div className="flex flex-wrap gap-3 items-center pt-4">
                                <label className="flex items-center gap-1 text-2xs text-gray-600 dark:text-gray-400 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={!!widgetConfig.noFrame}
                                    onChange={(e) => updateWidgetProperty('noFrame', e.target.checked)}
                                ...
                                <label className="flex items-center gap-1 text-2xs text-gray-600 dark:text-gray-400 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={!!widgetConfig.noBg}
                                    onChange={(e) => updateWidgetProperty('noBg', e.target.checked)}
                                    className="rounded border-gray-300 dark:bg-gray-800"
                                  />
                                  <span>No BG</span>
                                </label>
                          ...
                          {/* Preview container */}
                          {previewUrl && (
                            <div className="pt-2 flex justify-center bg-white dark:bg-black/30 p-3 rounded border border-gray-100 dark:border-gray-900 overflow-auto">
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img
                                src={previewUrl}
                                ...
                                loading="lazy"
                                onError={(e) => {
                                  // Snake raw file may not exist yet if workflow isn't run. Fallback gracefully.
